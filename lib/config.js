    ...
    const base = {
      data: process.env.XDG_DATA_HOME ||
            (this.platform === 'darwin'
              ? path.join(this.homeDir, 'Library', 'Application Support', 'podme')
              : path.join(this.homeDir, '.local', 'share', 'podme')),
      config: process.env.XDG_CONFIG_HOME ||
              (this.platform === 'darwin'
                ? path.join(this.homeDir, 'Library', 'Application Support', 'podme', 'config')
                : path.join(this.homeDir, '.config', 'podme')),
      cache: process.env.XDG_CACHE_HOME ||
             (this.platform === 'darwin'
               ? path.join(this.homeDir, 'Library', 'Caches', 'podme')
               : path.join(this.homeDir, '.cache', 'podme'))
    };
