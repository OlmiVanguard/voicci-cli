  ...
  getRecommendations(level, stats) {
    ...
    if (level === 'critical') {
      ...
      recommendations.push({
        ...
        command: 'podme config set-profile low'
      });

      recommendations.push({
        action: 'restart',
        message: 'Consider restarting the worker process to free memory',
        command: 'pkill -f "node.*worker.js" && podme <file>' // Restart worker
      });
    } else if (level === 'warning') {
      recommendations.push({
        action: 'profile',
        message: 'Consider switching to a lower memory profile',
        command: 'podme config set-profile medium' // Assume currently on high
      });

      recommendations.push({
        action: 'jobs',
        message: 'Reduce concurrent job limit',
        command: 'podme config set-jobs 1'
      });

      recommendations.push({
        action: 'monitoring',
        message: 'Enable memory monitoring if disabled',
        command: 'podme config set-monitoring on'
      });
    }
