...
const CONFIG_DIR = path.join(XDG_CONFIG_HOME, 'podme');
  ...
  getRecommendations() {
    ...
    if (this.config.memoryProfile !== recommended) {
      recommendations.push({
        ...
        command: `podme config set-profile ${recommended}`
      });
    }
    ...
    if (recommended === 'high' && settings.enableMemoryMonitoring) {
      recommendations.push({
        ...
        command: 'podme config set-monitoring off'
      });
    ...
    if (recommended === 'low' && this.config.qualityPreset === 'best') {
      recommendations.push({
        ...
        command: 'podme config set-quality balanced'
      });
    }
